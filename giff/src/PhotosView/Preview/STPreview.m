...
- (void)initControlsVisibles:(BOOL)useForCamera {
    ...
    if(useForCamera){
        ...
        self.postFocusSliderView.visible = [STGIFFApp postFocusAvailable];
...
- (void)setInteraction:(BOOL)useForCamera; {
    ...
    _tapAndPinchTarget = [STGIFFApp isInSimulator] ? _simulationCameraView : _filterCollectionView.contentView;
    _panTarget = [STGIFFApp isInSimulator] ? _simulationCameraView : _filterCollectionView;
    _resetIsRunning = NO;
...
- (void)unsubscribeWhenUsingCamera {
    ...
    [[STGIFFAppSetting get] whenValueOf:@keypath([STGIFFAppSetting get].postFocusMode) id:@"STPreview.observe.postFocusMode" changed:nil];
...
- (STPostFocusMode)currentFocusMode{
    ...
    if(_useForCamera){
        focusMode = (STPostFocusMode) STGIFFAppSetting.get.postFocusMode;
    }else{
...
#pragma mark TutorialGuide - Focus Point
#if DEBUG
    BlockOnce(^{
        STGIFFAppSetting.get._confirmedTutorialPointedFocalPoint = NO;
    });
#endif
    BOOL focusGuideWasShowen = NO;
    if(!STGIFFAppSetting.get._confirmedTutorialPointedFocalPoint){
    ...
    UITapGestureRecognizer * singleTap = [_tapAndPinchTarget whenTappedParams:^(UIGestureRecognizer *sender, UIGestureRecognizerState state, CGPoint location) {
#pragma mark TutorialGuide - Focus Point
        if(focusGuideWasShowen && !STGIFFAppSetting.get._confirmedTutorialPointedFocalPoint){
...
- (void)finishTutorialAction:(BOOL)userConfirm{
    ...
    self.verticalFocusPointerView.alpha = .5;
    if(userConfirm){
        STGIFFAppSetting.get._confirmedTutorialPointedFocalPoint = YES;
