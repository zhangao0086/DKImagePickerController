...
- (void)setContents {
    ...
    if(!_gridView){
        ...
        [elieLogo whenTapped:^{
            ...
            [[self st_rootUVC] openURL:[STGIFFApp elieSiteUrl] fallbackUrl:nil relatedView:nil];
        }];
...
#if DEBUG
        titleIconView.subtitleText =[NSString stringWithFormat:@"%@ %@", [STGIFFApp appVersion],[STGIFFApp buildVersion]];
#else
        titleIconView.subtitleText =[NSString stringWithFormat:@"%@", [STGIFFApp appVersion]];
#endif
    ...
    switch(item){
        ...
        case STAppInfoItem_CS_Feedback:
            ...
            p.url = [[NSString stringWithFormat:@"mailto:ask@postfoc.us?subject=%@ %@,&body=\n\n\n%@\n%@"
                    ...
                    , [STGIFFApp displayName]
                    , NSLocalizedString(@"main.appinfo.feedback.noti",nil)
                    , [NSString stringWithFormat:@"%@|%d|%d|%d"
                            , STGIFFAppSetting.get._guid
                            , STGIFFAppSetting.get.postFocusMode
                            , [STGIFFAppSetting get].afterManualCaptureAction
                            , [STGIFFAppSetting get].captureOutputSizePreset
                    ]
            ] escapeForQuery];
        ...
        case STAppInfoItem_CS_Note:
            ...
            p.url = [STGIFFApp releaseNoteUrl];
            p.blockForCreated = ^(UIView *targetView, STAppInfoItemParam *paramSelf) {
                if([targetView isKindOfClass:STStandardButton.class]){
                    ...
                    if([[STGIFFAppSetting get] isFirstLaunchSinceLastBuild]){
                        targetButton.badgeText = STStandardButtonsBadgeTextNew;
                    }else{
                        [[STGIFFAppSetting get] checkNewReleaseNoteIfPossible:^(BOOL updated) {
                            if (updated) {
                                targetButton.badgeText = STStandardButtonsBadgeTextNew;
                            }
                        }];
            ...
            p.blockForTapped = ^(UIView *targetView, STAppInfoItemParam *paramSelf) {
                ...
                if(targetButton.badgeText){
                    ...
                    [[STGIFFAppSetting get] touchNewReleaseNoteIfNeeded];
                }
            };
            break;
        case STAppInfoItem_CS_LinkAppStore:
            ...
            p.url = [NSURL URLForAppstoreApp:[STGIFFApp appstoreId]].absoluteString;
            break;
        case STAppInfoItem_CS_Tell:{
            ...
            p.blockForTapped = ^(UIView *targetView, STAppInfoItemParam *paramSelf) {
                Wself.shareController = [[UIActivityViewController alloc] initWithActivityItems:@[
                        [NSString stringWithFormat:@"%@ %@", [STGIFFApp marketingTitle], [STGIFFApp siteUrl]],
                        [NSURL URLForAppstoreWeb:[STGIFFApp appstoreId]],
...
//            p.url = [[NSString stringWithFormat:@"sms:?body=Elie\n%@&attachments=%@", [NSURL URLForAppstoreWeb:[STGIFFApp appstoreId]], [STGIFFApp iconPathOrCreateToPublish]] escapeForQuery];
        }
            break;
        ...
        case STAppInfoItem_Site_Facebook:
            ...
            p.fallbackUrl = [STGIFFApp urlToForwardSNSService:@"facebook"];
            break;
...
//            p.title = NSLocalizedString(@"main.appinfo.about", @"");
            p.url = [STGIFFApp localizedSiteUrl];
            break;
        ...
        case STAppInfoItem_System_RestoreAll:{
            ...
            p.selected = [STGIFFApp canTryRestoreAll];
            ...
            p.blockForTapped = p.disabled ? nil : ^(UIView * __weak targetView, STAppInfoItemParam * __weak paramSelf) {
                ...
                [STGIFFApp restoreAllProductIfNeeded:^(NSArray *transactions) {
                ...
                [STGIFFApp logUnique:@"RestoreAll"];
            };
        }
            break;
...
//            p.url = [NSURL URLForAppstoreApp:[STGIFFApp paidElieAppstoreId]].absoluteString;
            p.blockForTapped = ^(UIView *targetView, STAppInfoItemParam *paramSelf) {
                [STGIFFApp logUnique:@"BuyAll"];
            };
        }
            break;
        default:
...
//    button.titleLabelPositionedGapFromButton = style==STStandardButtonStylePTTP ? 0 : [STStandardLayout sizeBulletBig].height;
    ...
    [button whenSelected:^(STSelectableView * selectedView, NSInteger index) {
        ...
        [STGIFFApp logEvent:@"AppInfoItem" key:[@(param.tag) stringValue]];
    }];
...
- (void)expressWhenPresentIfNeeded:(BOOL)force {
    if(force || [[STGIFFAppSetting get] isFirstLaunchSinceLastBuild]){
