...
//    effectView.maskView = self.st_shadow;
    ...
    manualAfterCaptureCollectable.currentMappedValue = @(STGIFCAppSetting.get.afterManualCaptureAction);
    [manualAfterCaptureCollectable whenSelectedWithMappedValue:^(STSelectableView *button, NSInteger index, id value) {
        ...
        switch ([STGIFCAppSetting get].afterManualCaptureAction = [value integerValue]){
    ...
    resolutionCollectable.currentMappedValue = @([self setCaptureSizePreset:(CaptureOutputSizePreset) [[STGIFCAppSetting.get read:@keypath([STGIFCAppSetting get].captureOutputSizePreset)] integerValue] userTapped:NO]);
    //postFocusMode -> captureOutputSizePreset -> currentMappedValue
    [[STGIFCAppSetting get] whenValueOf:@keypath([STGIFCAppSetting get].postFocusMode) id:@"postFocusMode_maincontrol" changed:^(id value, id _weakSelf) {
        [self setCaptureSizePreset:(CaptureOutputSizePreset) [[STGIFCAppSetting.get read:@keypath([STGIFCAppSetting get].captureOutputSizePreset)] integerValue] userTapped:NO];
    }];
    ...
    [[STGIFCAppSetting get] whenValueOf:@keypath([STGIFCAppSetting get].captureOutputSizePreset) id:@"captureOutputSizePreset_maincontrol" changed:^(id value, id _weakSelf) {
...
- (CaptureOutputSizePreset)setCaptureSizePreset:(CaptureOutputSizePreset)preset userTapped:(BOOL)userTapped {
    ...
    [STGIFCAppSetting get].captureOutputSizePreset = [STPostFocusCaptureRequest restrictCaptureOutputSizePresetByPostFocusMode:(STPostFocusMode) [STGIFCAppSetting get].postFocusMode
                                                                                                                  targetPreset:preset
...
#pragma mark Product - Save
    ...
    [[STExportManager sharedManager] export:exportType will:^(STExporter *exporter) {
        ...
        exporter.hashtags = @[[STGIFCApp primaryHashtag]];
        ...
        if(result == STExportResultSucceed){
            STGIFCAppSetting.get.exportedType = exportType;
        }
        ...
        [STGIFCApp logEvent:@"ExportFinishResult" key:[@(result) stringValue]];
    }];

    [STGIFCApp logEvent:@"ExportTryType" key:[@(exportType) stringValue]];
}
...
- (STExportType)targetExportType{
    ...
    STExportType type = (STExportType) [STGIFCAppSetting get].exportedType;
...
- (void)requestQuickPostFocusCaptureIfPossible:(STPostFocusMode)mode{
    Weaks
    if([STGIFCApp afterCameraInitialized:@"STMainControl.quickaction.requestQuickPostFocusCaptureIfPossible" perform:^{
        Strongs
...
- (void)requestQuickCaptureIfPossible {
    Weaks
    if([STGIFCApp afterCameraInitialized:@"STMainControl.quickaction.quickcapture" perform:^{
        Strongs
        [Sself->_home cancelRestoreStateEffect];
...
- (BOOL)selectFilter:(NSString *)productId
    ...
    return [STGIFCApp selectByValue:productId
...
- (void)selectFilterItemFromSelectableItemAt:(NSInteger)selectedIndex{
    ...
    [STGIFCApp logClick:@"FilterSelectByHomeSelectable" key:filterItem.uid_short];
}
...
- (BOOL)selectFilterItemFromHomeButton:(void (^)(id resultValue, BOOL success))completionBlock{
    ...
    [STGIFCApp logClick:[@"FilterSelectByHomeButtonAtMode" st_add:[@(_mode) stringValue]] key:filterItem.uid_short];
