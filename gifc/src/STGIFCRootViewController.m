...
@interface STGIFCRootViewController (){
    //Device Control
...
@implementation STGIFCRootViewController
...
//    _controlBoardController.view.hidden = YES;
    ...
    _photoSelectionView.backgroundColor = [STGIFCApp launchScreenBackgroundColor];
...
- (void)cameraPreviewDidInitialized{
    ...
    //load photos
    STGIFCAppSetting.get.photoSource = STPhotoSourceCapturedImageStorage;
...
//    if([[STGIFCAppSetting get] isFirstLaunchSinceLastBuild] && [STSettingScreenControllableItem hasAddedUntouchedItems]){
...
- (void)initRootListeners {
    ...
    [[STGIFCAppSetting.get st_propertyNames] bk_each:^(id obj) {
        [weakSelf_1 _applySettings:obj value:[STGIFCAppSetting.get valueForKeyPath:obj] initializing:YES];
    }];

    WeakSelf weakSelf = self;
    [STGIFCAppSetting.get whenSavedToAll:[@(self.hash) stringValue] withBlock:^(NSString *key, id value) {
        [weakSelf _applySettings:key value:value];
        /*
            * update context if not private value
        */
        if(![STGIFCAppSetting isKeyPathForInternalValue:key]){
    ...
    [[STUserActor sharedInstance] when:^(NSInteger action, id object) {
        if (action == STUserActionChangeCameraMode) {
            ...
            [Wself setElieMode:STGIFCAppSetting.get.elieMode == STElieModeFace ? STElieModeMotion : STElieModeFace];
        }
    }];
...
- (void)_applySettings:(NSString *)keypath value:(id)value initializing:(BOOL)initializing {
    STGIFCAppSetting * p = STGIFCAppSetting.get;
...
- (BOOL)context_needsFaceDetection {
    ...
    if(STGIFCAppSetting.get.elieMode != STElieModeFace){
        return NO;
    }
...
#pragma mark Product - Save
    if(STCameraModeManualExitAndPause==mode){
        if(![STGIFCApp tryProductSavePostFocus:^(BOOL purchased) {
            if(purchased){
                [self setCameraMode:STCameraModeManualExitAndPause];
            }
    ...
    STGIFCAppSetting.get.mode = mode;
...
//                [selfObject captureAsElie];
    ...
    STGIFCAppSetting.get.elieMode = mode;
...
//    BOOL duplicatedChange = [STElieCamera mode] != STCameraModeNotInitialized && mode == [[STSetting.get read:@keypath([STSetting get].performanceMode)] integerValue];
    ...
    STGIFCAppSetting.get.captureSessionPreset = preset;
}

- (void)applyCaptureSessionPresetFromSetting {
    NSString * preset = [STGIFCAppSetting.get.captureSessionPreset copy];
...
//        [[STPhotoSelector sharedInstance] doAfterCaptured:[STPhotoItemSource sourceWithImage:[UIImage imageNamed:@"LaunchScreenIcon"]]];
    ...
    switch (STElieCamera.mode){
        case STCameraModeElie:{
            switch ((EliePerformanceMode) [[STGIFCAppSetting.get read:@keypath([STGIFCAppSetting get].performanceMode)] integerValue]){
                case EliePerformanceModeSingle:
        ...
        case STCameraModeManual:{
            switch ((STAfterManualCaptureAction) [[STGIFCAppSetting.get read:@keypath([STGIFCAppSetting get].afterManualCaptureAction)] integerValue]){
    ...
    request.privacyRestriction = STGIFCAppSetting.get.securityModeEnabled;
    request.tiltShiftEnabled = STGIFCAppSetting.get.tiltShiftEnabled;
    request.autoEnhanceEnabled = STGIFCAppSetting.get.autoEnhanceEnabled;
...
- (void)captureAnimatable:(STAnimatableCaptureRequest *)request {
    ...
    request.responseHandler = ^(STCaptureResponse *result) {
        ...
        if(result){
            STGIFCAppSetting.get.afterManualCaptureAction = STAfterManualCaptureActionEnterAnimatableReview;
...
#pragma mark Product
    ...
    [STCapturedImage registerProcessingBlockBeforeSave: [STGIFCApp isPurchasedProduct:STGIFCProduct_save] ? nil : ^UIImage *(UIImage *sourceImage) {
        if(![STGIFCApp isPurchasedProduct:STGIFCProduct_save]) {
            @autoreleasepool {
                NSString * cacheKey = @"STGIFCProduct_save_waterMarkImage";

                if([STApp isDebugMode] || [STGIFCAppSetting.get isFirstLaunchSinceLastBuild]){
...
//                    (null) > performed : 0.007762s at ckTime:
    ...
    //image size
    request.captureOutputSizePreset = (CaptureOutputSizePreset) STGIFCAppSetting.get.captureOutputSizePreset;
    ...
    request.responseHandler = ^(STCaptureResponse *result) {
        ...
        if(result){
//            STGIFCAppSetting.get.afterManualCaptureAction = STAfterManualCaptureActionEnterEdit;
    ...
    //focal point
    switch((STPostFocusMode)[STGIFCAppSetting get].postFocusMode){
        ...
        if(_lockCaptureInterval==NSTimeIntervalSince1970 && _lockedCapture){
            ...
            if([STElieCamera sharedInstance].isPositionBack){
                if(_needsAlwaysRequestFocusWhenFaceIn || _initialRequestFocusWhenFaceIn){
                    ...
                    if([[[STGIFCAppSetting get] read:@keypath([STGIFCAppSetting get].enabledBacklightCare)] boolValue]){
...
//    _haptic.actionFlag = (STHapticAction)[[STGIFCAppSetting.get read:@keypath([STGIFCAppSetting get].hapticAction)] integerValue];
//
