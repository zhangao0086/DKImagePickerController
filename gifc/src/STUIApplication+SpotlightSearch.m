...
NSString * const STSearchableItemDomainDefaultLocalizableKeyword = @"stells_gifc_domain_DefaultLocalizableKeyword";
NSString * const STSearchableItemDefaultLocalizableKeywordIdentifier = @"stells_gifc_id_DefaultLocalizableKeyword";
...
//quick action items
NSString * const STSearchableItemDomainQuickActions = @"stells_gifc_domain_QuickActions";
NSString * const STSearchableItemIdentifierPrefixQuickActions = @"stells_gifc_id_QuickActions";

//controllable items
NSString * const STSearchableItemDomainControllable = @"stells_gifc_domain_Controllable";
NSString * const STSearchableItemIdentifierPrefixControllable = @"stells_gifc_id_Controllable";

//presets
NSString * const STSearchableItemDomainPreset = @"stells_gifc_domain_Preset";
NSString * const STSearchableItemIdentifierPrefixPreset = @"stells_gifc_id_Preset";
...
- (BOOL)launchFromNeededSearchableItemByUserActivityIfPossible:(NSUserActivity *)userActivity{
    ...
    @try {
        ...
        [STGIFCApp logEvent:@"LaunchFromSpotlightSearchForPrefix" key:prefix];
...
- (BOOL)isPossibleIndexing{
    return [STGIFCApp osVersion].majorVersion>=9 && [CSSearchableIndex isIndexingAvailable];
}

- (BOOL)isPossibleIndexingOnlyNewVersion{
    return [self isPossibleIndexing] && [STGIFCAppSetting.get isFirstLaunchSinceLastBuild];
}

- (BOOL)isPossibleIndexingNewVersionOrSinceLastLaunchAt6Hours{
    return [self isPossibleIndexing] && ([STGIFCAppSetting.get isFirstLaunchSinceLastBuild] || [STGIFCAppSetting.get isPassedHoursSinceLastLaunched:6]);
}
...
- (void)indexDefaultSearchableItemsIfPossible {
    ...
    Weaks
    if([STGIFCApp afterCameraInitialized:@"STSearchableIndexing.default" perform:^{
    ...
    dispatch_async([STQueueManager sharedQueue].startingBackground, ^{
        ...
        attributeSet_default.title = [STGIFCApp displayName];
        ...
        [[CSSearchableIndex defaultSearchableIndex] indexSearchableItems:seachableItems completionHandler: ^(NSError * __nullable error) {
            if (!error){
                ...
                [STGIFCAppSetting.get touchSearchableContext:STSearchableContextDefault];
            }
        }];
    });
}
...
//    }
    ...
    if([STGIFCApp afterCameraInitialized:@"STSearchableIndexing.controllable" perform:^{
