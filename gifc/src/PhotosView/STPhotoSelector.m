...
- (void)doSlideUp:(BOOL)up{
    [STStandardUX setAnimationFeelToRelaxedSpring:self];

    CGFloat lastScrollOffsetY = _gridView.contentSize.height- (_gridView.bounds.size.height-_gridView.contentInsetBottom);
    CGFloat gapFromBottom = CLAMP((lastScrollOffsetY-_gridView.contentOffsetY),0,_gridView.contentInsetBottom);
    CGFloat additionalOffsetY = 0;

    //TODO : 하드코드 말고 flexible layout
//    if([STGIFCApp screenFamily]==STScreenFamily4){
//        additionalOffsetY = [STElieStatusBar sharedInstance].height*1.6f;
//    }else{
//        additionalOffsetY = [STElieStatusBar sharedInstance].height;
//    }

    if(up){
        self.spring.y = -(gapFromBottom + additionalOffsetY);
        [self st_setShadow:UIRectEdgeBottom size:20 shadowColor:nil rasterize:YES strong:NO atIndex:0];
        [self st_shadow].y -= _gridView.contentInsetBottom - gapFromBottom;

        [self whenLongTapAsTapDownUp:^(UILongPressGestureRecognizer *sender, CGPoint location) {
            [sender.view whenLongTapped:nil];
...
- (void)doAfterCaptured:(STPhotoItemSource *)photoItemSource; {
    ...
    if(STElieCamera.mode == STCameraModeManual || STElieCamera.mode == STCameraModeManualWithElie){
        NSInteger action = STGIFCAppSetting.get.afterManualCaptureAction;
        ...
        NSInteger perfMode = STGIFCAppSetting.get.performanceMode;
    ...
    switch(self.source){
        ...
        case STPhotoSourceRoom:{
            ...
            if(self.gridView.items.count){
                ...
                STPhotoItem * lastestRoomItem = [self.gridView.items st_objectOrNilAtIndex:STGIFCAppSetting.get._lastCapturedIndexInRoom];
...
- (void)doEnterEditByItem:(STPhotoItem *)targetItem {
    ...
    _previewCollector.targetPhotoItem.needsEnhance = STGIFCAppSetting.get.autoEnhanceEnabledInEdit;
...
- (STPhotoSource)source; {
    return (STPhotoSource) [[[STGIFCAppSetting get] read:@keypath([STGIFCAppSetting get].photoSource)] integerValue];
}
...
- (void)_changeSource:(STPhotoSource)source effect:(BOOL)effect{
    ...
    void(^loadSource)(void) = ^{
        STGIFCAppSetting.get.photoSource = source;
    ...
    if(effect){
        if([STGIFCApp isCurrentDeviceInLowRenderingPerformanceFamily]){
            Weaks
...
- (void)requestOpenAlbumIfPossible {
    ...
    if([STGIFCApp afterCameraInitialized:@"STPhotoSelector.quickaction.openalbum" perform:^{
        [Wself requestOpenAlbumIfPossible];
    }]){
        return;
    }
...
- (void)initialShowing{
    ...
    self.superview.backgroundColor = [STGIFCApp launchScreenBackgroundColor];
...
- (void)loadFromCurrentSource {
    STPhotoSource source = (STPhotoSource) STGIFCAppSetting.get.photoSource;
    NSAssert(self.source==source, @"STGIFCAppSetting.get.photoSource == [STPhotoSelector sharedInstance].source");
...
- (void)_putPhotoItemsFromRoom:(NSArray *)items{
    ...
    NSInteger preferedIndex = STGIFCAppSetting.get._lastCapturedIndexInRoom;
    ...
    for(STPhotoItem * item in items){
        if(self.gridView.items.count < STGIFCAppSetting.get.currentRoomSize){
...
- (NSUInteger)nextRoomIndex:(NSUInteger)currentPuttedIndex{
    return currentPuttedIndex<0 || currentPuttedIndex >= STGIFCAppSetting.get.currentRoomSize -1 ? 0 : currentPuttedIndex +1;
}

- (NSUInteger)nextLastCaptureIndexInRoom {
    NSUInteger index = self.gridView.items.count < STGIFCAppSetting.get.currentRoomSize ? self.gridView.items.count : [self nextRoomIndex:(NSUInteger) STGIFCAppSetting.get._lastCapturedIndexInRoom];
...
- (void)saveLastCapturedIndex:(NSInteger) index{
     STGIFCAppSetting.get._lastCapturedIndexInRoom = index;
     if(![STGIFCAppSetting.get shouldAutomaticallySynchronize]){
...
#pragma clang diagnostic pop
        ...
        case STPhotoSourceRoom:
        {
            ...
            dispatch_async(DefaultWritingIOQueue, ^{
                ...
                [_self saveImageToUrl:[photo scaleToFitSize:[STGIFCApp memorySafetyRasterSize:[_self previewImageSizeByType:STPhotoViewTypeDetail]]] fileUrl:fullscreenUrl quality:.8];
...
- (void)_writeToTempAndCreateItem:(STPhotoItemSource *)photoSource completion:(void(^)(STPhotoItem *))block{
    ...
    dispatch_async([STQueueManager sharedQueue].writingIO, ^{
        ...
        [_self saveImageToUrl:[defaultImage scaleToFitSize:[STGIFCApp memorySafetyRasterSize:[_self previewImageSizeByType:STPhotoViewTypeDetail]]] fileUrl:fullscreenUrl quality:.8 background:NO];

        NSURL *previewUrl = [_self makeTempImageSaveUrl:kSTImageFilePrefix_TempToEdit_PreviewImage];
        UIImage *previewImage = [defaultImage scaleToFitSize:[STGIFCApp memorySafetyRasterSize:[_self previewImageSizeByType:STPhotoViewTypeGridHigh]]];
...
//    }
    ...
    dispatch_async([STQueueManager sharedQueue].readingIO, ^{
        ...
        [@(0) upto:STGIFCAppSetting.get.currentRoomSize -1 do:^(NSInteger index) {
...
- (void)_test_loadAndPutThumbnailsFromRoom{
    ...
    for(NSInteger i=0; i< STGIFCAppSetting.get.currentRoomSize; i++){
...
- (void)updateExportedStateIfNeeded:(NSURL *)assetUrl{
    ...
    if(STPhotoSourceRoom==self.source){
        [[STGIFCAppSetting get] savePhotosOrigin:assetUrl origin:STPhotoItemOriginExportedFromRoom];
...
- (void)loadExportedStateIfNeededWhenChangedSource {
    if(STPhotoSourceRoom==self.source){
        NSArray * photoUrls = [[STGIFCAppSetting get] photoUrlsByOrigin:STPhotoItemOriginExportedFromRoom];
        if(photoUrls.count){
        ...
        [[STGIFCAppSetting get] clearPhotosOrigins];
    }
}
