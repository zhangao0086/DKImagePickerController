...
+(void)saveImageToCameraRoll:(UIImage*)image location:(CLLocation*)location completionBlock:(PHAssetAssetBoolBlock)completionBlock{
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        if(success){
            ...
            !completionBlock?:completionBlock(asset, YES);
        } else {
            !completionBlock?:completionBlock(nil, NO);
        }
        placeholderAsset = nil;
    }];
}

+(void)saveImageToCameraRollAtURL:(NSURL *)imageURL location:(CLLocation *)location completionBlock:(PHAssetAssetBoolBlock)completionBlock{
    __block PHObjectPlaceholder *placeholderAsset = nil;
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
